...
const AddMovieModal = ({ isOpen, onClose, onAddMovie }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const newMovieData = {
      title: movieTitle.trim(), year: parseInt(movieYear, 10),
      type: movieType.trim(), country: movieCountry.trim(),
      duration: parseInt(movieDuration, 10),
      director: movieDirector.trim(), language: movieLanguage.trim(),
      description: movieDescription.trim(),
      newPosterFile: posterFile,
    };
    onAddMovie(newMovieData);
  };
